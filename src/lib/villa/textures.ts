 ...
 *
 * Three rules keep them from reading as computer graphics:
 ...
 *
 * Each generator returns a colour map and a height map; the height map also drives roughness.
 */
...
const rgb = (hex: string): RGB => {
  ...
  // Work in sRGB bytes: the canvas is an sRGB surface.
...
function canvas(width: number, height = width) {
  ...
  el.width = width;
  el.height = height;
  return { el, ctx: el.getContext('2d', { willReadFrequently: false })! };
}

function toTexture(el: HTMLCanvasElement, { srgb = true }: { srgb?: boolean } = {}) {
...
/** One pass over the pixels fills both the colour and the height canvas. */
function build(key: string, size: number, tile: number, shade: (u: number, v: number) => { colour: RGB; height: number }): Pair {
  const mapKey = `${key}:map`;
  const bumpKey = `${key}:bump`;
  if (!cache.has(mapKey)) {
    const a = canvas(size);
    const b = canvas(size);
    const colour = a.ctx.createImageData(size, size);
    const height = b.ctx.createImageData(size, size);
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        const sample = shade(x / size, y / size);
        const i = (y * size + x) * 4;
        colour.data[i] = sample.colour[0];
        colour.data[i + 1] = sample.colour[1];
        colour.data[i + 2] = sample.colour[2];
        colour.data[i + 3] = 255;
        const g = clamp01(sample.height) * 255;
        height.data[i] = height.data[i + 1] = height.data[i + 2] = g;
        height.data[i + 3] = 255;
      }
    }
    a.ctx.putImageData(colour, 0, 0);
    b.ctx.putImageData(height, 0, 0);
    cache.set(mapKey, toTexture(a.el));
    cache.set(bumpKey, toTexture(b.el, { srgb: false }));
  }
  return { map: cache.get(mapKey)!, bump: cache.get(bumpKey)!, tile };
}
...
export type WoodTone = 'light-oak' | 'dark-oak' | 'walnut' | 'natural-oak';

export function wood(tone: WoodTone, { planks = false }: { planks?: boolean } = {}): Pair {
  const palette = {
  ...
  return build(`wood-${tone}-${planks ? 'planks' : 'veneer'}`, 1024, TILE.wood, (u, v) => {
    let uu = u;
    let lift = 0;
    let joint = 0;
    if (planks) {
...
export function marble(tone: 'white' | 'beige' | 'dark'): Pair {
  ...
  return build(`marble-${tone}`, 1024, TILE.marble, (u, v) => {
...
export type StoneTone = 'travertine' | 'limestone' | 'dark-stone' | 'concrete';
...
export function stone(tone: StoneTone): Pair {
  const noise = makeNoise(tone.length * 13);
  return build(`stone-${tone}`, 1024, TILE.stone, (u, v) => stoneSample(tone, noise, u, v));
}

/** Laid stone: slabs in running bond with an open joint, each slab a shade of its own. */
function laid(key: string, tone: StoneTone, tile: number, courses: number, perCourse: number, jointWidth: number, variation: number): Pair {
  const noise = makeNoise(tone.length * 13 + courses);
  return build(key, 1024, tile, (u, v) => {
...
export function paving(tone: 'travertine' | 'limestone' | 'dark-stone'): Pair {
  return laid(`paving-${tone}`, tone, TILE.paving, 4, 2, 0.0022, 0.09);
}

/** Facade cladding: 1200 × 600 sawn panels on a tight joint. */
export function cladding(tone: 'limestone'): Pair {
  return laid(`cladding-${tone}`, tone, TILE.cladding, 4, 2, 0.0014, 0.07);
}

/** Board-marked concrete: the grain and the lips of 150 mm formwork boards, and the tie holes. */
export function boardConcrete(): Pair {
  const palette = ['#5a5955', '#4b4a47', '#33322f'].map(rgb) as [RGB, RGB, RGB];
  const noise = makeNoise(71);
  const boards = 16;
  return build('board-concrete', 1024, TILE.boards, (u, v) => {
...
export function plaster(hex: string): Pair {
  ...
  return build(`plaster-${hex}`, 512, TILE.plaster, (u, v) => {
...
export function fabric(hex: string): Pair {
  ...
  const threads = 96;
  return build(`fabric-${hex}`, 512, TILE.fabric, (u, v) => {
...
/** Water surface ripple, as a tiling normal map. */
  ...
  if (!cache.has(key)) {
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        const u = x / size;
        const v = y / size;
        const h = fbm(noise, u, v, 8, 8, 4);
        const hx = fbm(noise, u + step * 2, v, 8, 8, 4);
        const hy = fbm(noise, u, v + step * 2, 8, 8, 4);
        const i = (y * size + x) * 4;
        data.data[i] = 128 + (h - hx) * 1400;
        data.data[i + 1] = 128 + (h - hy) * 1400;
        data.data[i + 2] = 255;
        data.data[i + 3] = 255;
      }
    }
